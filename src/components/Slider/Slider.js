...
class Slider extends PureComponent {
      ...
      if (user != null) {
        ...
        api
          ...
          .then((res) => {
              ...
              for (let i = 0; i < reminders.length; i++) {
                ...
                for (let x = dates.length - 1; x > -1; x--) {
                  ...
                  if (dates[x].reminderDate && dates[x].reminderTime) {
                    dt = moment(
                      dates[x].reminderDate.split('T')[0] +
                        ' ' +
                        dates[x].reminderTime,
                    ).format();
                  }
            ...
            // console.log('beforeSorting', data);

            // data = data.sort((a, b) =>
            //   moment(a.reminderDate) < moment(b.reminderDate) ? 1 : -1,
            // );

            data = data.sort(
              (a, b) =>
                moment(a.reminderDate).format('YYYYMMDD') -
                moment(b.reminderDate).format('YYYYMMDD'),
            );
  ...
  _renderItem({item, index}) {
    ...
    return (
      <TouchableOpacity
        onPress={() => RootNavigation.navigate('MyReminder')}
        style={{
          backgroundColor: '#fff',
          borderWidth: 1,
          borderColor: '#fff',
        ...
        <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
          <TouchableOpacity>
            <Image
              source={image ? {uri: image} : icons.AddPhoto}
        ...
        <View style={styles.nameContainer}>
          <View style={styles.nameAndphoto}>
            ...
            <Text style={styles.name}>
              {user != null ? user.userSession.fullName : ''}
            </Text>
          </View>
