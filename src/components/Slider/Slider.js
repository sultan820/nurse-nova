...
class Slider extends PureComponent {
        ...
        AsyncStorage.getItem("loginData").then(user => {
            if (user != null) {
                ...
                api.get(req).then(res => res.json()).then(res => {
                    ...
                    // moment('01-02-2020', 'DD-MM-YYYY').valueOf()
                    data = data.sort((a, b) => (moment(a.date) < moment(b.date)) ? 1 : -1)
                    console.log("\n\n\n\ndatatata", JSON.stringify(data, null, 4))
                    this.setState({
                        data
                    })
                })
        ...
        return (
            <TouchableOpacity onPress={() => RootNavigation.navigate('MyReminder')}
                ...
                <View style={{ flex: 3 }}>
                    <View style={{ flex: 1, flexDirection: 'row' }} >
                        <View style={{ flex: 1.5, justifyContent: 'center' }} >
                            <LinearGradient
                                ...
                                style={{ height: 30, flexDirection: 'row', paddingLeft: 10, paddingRight: 10, justifyContent: 'center', alignItems: 'center', borderRadius: 5 }}
                                start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                            >
                                <Icon name="clock-o" type="FontAwesome" size={3} style={[{ color: theme.colors.white, fontSize: 18, marginRight: 0, paddingRight: 0 }]} />
                                <Text style={[styles.time, { color: '#fff', fontSize: 14, fontWeight: 'bold', marginLeft: 0, textAlign: 'left', paddingLeft: 4, fontFamily: 'OpenSans-Light' }]}>
                                    {moment(item.reminderTime, ["HH:mm:ss"]).format('hh:mm a')}
                                </Text>
                            </LinearGradient>
                        </View>
                        <View style={{ flex: 2, justifyContent: 'center' }}>
                            <Text style={[styles.day, { textTransform: 'uppercase', fontSize: 18, fontFamily: 'OpenSans-Light', fontWeight: 'bold' }]}>
                    ...
                    <View style={{ flex: 1, }}>
                        <Text style={[styles.pill, { fontFamily: 'OpenSans-Light', fontWeight: 'bold', marginBottom: 15 }]}>{item.otherMedicineDoze}</Text>
                    </View>
                </View>
    ...
    render() {
        console.log("\n\n\n\n\n\ndata for Carousel ", JSON.stringify(this.state.data, null, 4));
        return (
            <View style={{ flex: 1 }}>
                <Carousel
