...
export default function Language(props) {
  ...
  const getLanguage = () => {
    AsyncStorage.getItem('lang').then((lang) => {
      if (lang != null) setLang(JSON.parse(lang));
    });
  };
  const toggleSwitch = (value) => {
    Alert.alert(
      'Alert',
      'Kindly note that the app will now restart in order to load in another language, this will take few seconds.',
      [
        {
          text: 'Cancel',
        ...
        {text: 'OK', onPress: () => onToggleSwitch(value)},
      ],
      {cancelable: false},
    );
  };
