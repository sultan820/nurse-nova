...
export default function _MedicalRecordForm(props) {
  ...
  const getProps = () => {
    ...
    if (props.route.params.healthDiary != undefined) {
      props.navigation.setOptions({
        headerTitle: () => (
          <ClosingHeader {...props} title="Edit Health Diary" />
        ),
      });
  ...
  const renderOption = (title, index) => (
    <SelectItem key={index} title={title.title} />
  );
        ...
        <>
          <View style={Styles.mainContainer}>
            <View style={Styles.topContainer}>
              ...
              <View style={Styles.fieldContainer}>
                ...
                {displayValue !== undefined && displayValue.title == 'Other' && (
                  <View style={{marginTop: 5}}>
                    <Input
                      placeholder=""
                      onChangeText={(text) => setHealthDiaryTypeOther(text)}
                    />
                  </View>
                )}
              </View>
              ...
              {data.length == 0 && displayValue == undefined && (
                <View style={Styles.fieldContainer}>
                  <Select label="Select Type" value="Loading...">
