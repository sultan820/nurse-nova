...
function _MyHealthdiary(props) {
          ...
          <Layout
            ...
            <View>
              <Text
                onPress={() => {
                  setVisible(false);
                  props.navigation.navigate('AddNewMember', {
                    healthDiary: List.filter((l) => l.id == id),
                  });
                }}
                style={{
                  padding: 10,
        ...
        .then((res) => {
          ...
          Share.share({
            message:
              ...
              ' shared you health record. Check at ' +
              res.pdfLink,
            url: res.pdfLink,
          });
        });
    ...
    props.navigation.setOptions({
      headerTitle: () => (
        <MultiStackHeader
          ...
          multiDelete={confirmMutliDelete}
          title=""
        />
      ),
    });
  ...
  const renderRow = ({item, index}) => {
    ...
    return (
      <TouchableOpacity
        onPress={() =>
          props.navigation.navigate('HealthDiaryDetail', {record: item})
        }
          ...
          <View style={Styles.descriptionContainer}>
            {selectedCheckboxs.length > 0 ? (
              <View style={{width: 60}}>
                <CheckBox
                  checked={
                    selectedCheckboxs.indexOf(item.id) > -1 ? true : false
                  }
                  value={selectedCheckboxs.indexOf(item.id) >= 0 ? true : false}
                  onPress={(done) => {
                    addCheckbox(done, item.id);
                  }}
                />
              </View>
            ) : null}
  ...
  return (
    <View style={Styles.mainContainer}>
      {selectedCheckboxs.length > 0 ? (
        <View
          ...
          <View
            ...
            <CheckBox
              checked={List.filter(
                (data) => selectedCheckboxs.indexOf(data.id) <= -1,
              )}
              onPress={(done) => {
                callSelectAll();
              }}
      ...
      <Modal
        ...
        transparent
        onRequestClose={() => setMultiDeleteModal(false)}>
        <View
          style={{
            flex: 1,
            paddingHorizontal: 20,
