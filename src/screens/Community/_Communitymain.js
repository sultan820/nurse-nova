...
export default function _Communitymain(props) {
  ...
  const renderRow = ({item, index}) => {
    // console.log("\n\n\n\nitem.isFavoriteArticle", JSON.stringify(item, null, 4));
    ...
    return (
      <View
        ...
        <TouchableOpacity
          onPress={() => {
            props.navigation.navigate('CommunityDetail', {topic: item});
          }}
          onLongPress={() => {
          ...
          <Row>
            <Col style={{flex: 5}}>
              <Text style={{fontSize: 20, marginTop: 21}}>{item.title}</Text>
            </Col>
            <Col
              style={{
                alignItems: 'flex-end',
              ...
              {item.authorImage !== null ? (
                <Image
                  source={{uri: IMAGE_URL + item.authorImage}}
                  style={{
                    padding: 2,
                    borderRadius: 21,
                    height: 42,
