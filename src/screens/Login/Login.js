...
function Login(props) {
  ...
  const Login = () => {
    ...
    setLoading(true);
    auth
      .login({username, password, client_id: ID.clientId})
      .then((res) => res.json())
      .then((res) => {
  ...
  const error = () => {
    return (
      <Text style={Styles.error}>
        {err != null && err.message ? err.message : ''}
      </Text>
    );
  };
  ...
  return (
    <View style={Styles.mainContainer}>
      <View style={Styles.secondContainer}>
        ...
        <View style={Styles.formContainer}>{renderInputField(props)}</View>
      </View>
      <View style={Styles.buttonContainer}>
        <Button
          accessoryLeft={() => (loading ? LoadingIndicator() : null)}
          disabled={username !== '' && password !== '' ? false : true}
          onPress={!loading ? () => Login() : null}
          children={() => (
...
const mapDispatchToProps = (dispatch) => {
  return {
    login: (username, password, clientId) =>
      dispatch(action._Login(username, password, clientId)),
  };
};
