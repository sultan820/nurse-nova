...
export default function openclock(props) {
  ...
  const getProps = () => {
    ...
    props.navigation.setOptions({ headerTitle: () => <ClosingHeader {...props} title="Set Reminder Time " /> });
        ...
        <Content>
          <Row>
            <Col style={{ alignContent: 'center', alignItems: 'center' }}>
              ...
              <View style={{ justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
                ...
                <Animated.View {...panHandlerOuter.panHandlers} style={{ height: 300, width: 300, borderRadius: 120, transform: [{ rotate: angleOuter + 'deg' }] }}>
                  <Image source={minuteFace} resizeMode="contain" style={{ height: '100%', width: '100%' }} />
                </Animated.View>

                <Animated.View onLayout={layoutR => layout ? null : setLayout(layoutR)} {...panHandler.panHandlers} style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgb(67,111,149)', position: 'absolute', height: 180, width: 180, transform: [{ rotate: angle + 'deg' }], borderRadius: 120 }}>
                  <Image source={hourFace} resizeMode="contain" style={{ height: '100%', width: '100%', }} />
                </Animated.View>
          ...
          <Row >
            <Col style={{justifyContent:'center'}}>
              <Text style={{ fontSize: 30, color: "#596377" }} >{hours.toString().length === 1 ? `0${hours}` : hours}:{minutes.toString().length === 1 ? `0${minutes}` : minutes}</Text>
            </Col>
            <Col>
              <ToggleSwitch
