...
function _MedicalReminderList(props) {
              ...
              <Text
                onPress={() => {
                  ...
                  props.navigation.navigate('AddNewReminder', {
                    reminder: List.filter((l) => l.id == id),
                  });
                }}
                style={{
                  padding: 10,
                ...
                <Icon
                  name="edit-2"
                  type="Feather"
                  style={{fontSize: 14, color: '#596377'}}></Icon>{' '}
                Edit
              </Text>
              <Text
                onPress={() => {
  ...
  const renderRow = ({item, index}) => {
    ...
    if (index == 0) {
      ...
      // if (item.reminderDateTimes !== undefined && item.reminderDateTimes.length>0) {
      //     const date = item.reminderDateTimes[]
    ...
    return (
      <View style={Styles.recordContainer} key={index}>
        ...
        item.files[0].file_src !== '' &&
        item.files[0].file_src !== null ? (
          <Image
            source={{uri: IMAGE_URL + item.files[0].file_src}}
        ...
        <View
          ...
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={Styles.descriptionContainer}>
              ...
              {/* {item.otherMedicineDoze.trim() !== "" ? <Text style={Styles.doze}>{item.otherMedicineDoze}</Text> : <Text style={Styles.doze}>No pills</Text>} */}
              {(item.otherMedicineDoze !== undefined &&
                item.otherMedicineDoze !== '' && (
                  <Text style={Styles.doze}>{item.otherMedicineDoze}</Text>
                )) ||
                (item.medicineDozeSD !== undefined && (
                  <Text style={Styles.doze}>{item.medicineDozeSD} pills</Text>
                ))}
          ...
          <View style={Styles.bottomContainer}>
            <View style={Styles.dateContainer}>
              <Image
                source={require('../../assets/calendar-blank-outline.png')}
              ...
              {item.isLifetime ? (
                ...
                <Text>
                  {' '}
                  {item.reminderDateTimes[0] !== undefined &&
                    moment(item.reminderDateTimes[0].reminderDate).format(
                      'DD-MM-yyyy',
                    )}{' '}
                </Text>
              )}
              ...
              {item.isEveryday ? (
                <Text>
                  {' '}
                  {item.reminderDateTimes !== undefined &&
                    item.reminderDateTimes.length}{' '}
                  {item.reminderDateTimes.length === 1 ? 'time' : 'times'} per
                  day
                </Text>
              ) : (
