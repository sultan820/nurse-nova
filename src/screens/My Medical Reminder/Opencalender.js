...
export default function opencalendar(props) {
  ...
  const selectDate = (val) => {
    ...
    // if (val.dateString) {
    //     let d = [];
    //     d.push(...dates, {date: val.dateString});
    //     setDate(d);
    //   }
  };
      ...
      <Col style={{marginRight: 10}} key={index}>
        <Button
          ...
          <Icon
            name="close"
            type="FontAwesome"
            style={{fontSize: 15, color: '#fff'}}></Icon>
        </Button>
      </Col>
    );
  };
  ...
  const renderDate = ({item, index}) => {
    console.log('\n\n\n\nitem.time', item, '   typeof', typeof item.time);
    return (
      <Row
        key={index}
        style={{
        ...
        <Col style={{marginRight: 10}}>
          <Button
            ...
            style={{backgroundColor: '#888'}}>
            <Text>{moment(item.date).toDate().toDateString()} </Text>
            <Icon
              name="close"
              type="FontAwesome"
              style={{fontSize: 15, color: '#fff'}}></Icon>
                ...
                return (
                  ...
                  >
                    ...
                    <Text>
                      {' '}
                      {ite.time != null
                        ? moment(ite.time).format('hh:mm a')
                        : 'Add Time'}
                      {'  '}
                    </Text>
                    <Icon
                      ...
                      name="close"
                      type="FontAwesome"
                      style={{fontSize: 20, color: '#fff'}}></Icon>
                  </Button>
                );
              })
            : null}
  ...
  const selectTime = () => {
    ...
    if (clockId == 'all') {
      ...
      let dt = [{date: '', time: [{time: '', time: ''}]}];
      setTime([]);
      let newDates = dates;
      let time =
        newDates[clockId].time == undefined ? [] : newDates[clockId].time;
  ...
  const panHandlerOuter = PanResponder.create({
    ...
    onPanResponderMove: (evt, gestureHandler) => {
      ...
      setAngleOuter(angleOuter + dX * 0.7 + dY * 0.7);
    ...
    <Container style={{padding: 10}}>
      {!showClock && (
        <Content>
          ...
          <FlatList data={dates} renderItem={renderDate} horizontal={false} />
          ...
          <Row>
            <Col style={{alignContent: 'center', alignItems: 'center'}}>
              <View
                ...
                <Animated.View
                  {...panHandlerOuter.panHandlers}
                  style={{
                    height: 350,
                    width: 350,
                    borderRadius: 120,
                    transform: [{rotate: angleOuter + 'deg'}],
                  }}>
                  <Image
                    source={minuteFace}
                    resizeMode="contain"
                ...
                <Animated.View
                  onLayout={(layoutR) => (layout ? null : setLayout(layoutR))}
                  {...panHandler.panHandlers}
                  style={{
                    justifyContent: 'center',
                    alignItems: 'center',
                    backgroundColor: 'rgb(67,111,149)',
                    position: 'absolute',
                    height: 240,
                    width: 240,
